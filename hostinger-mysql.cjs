...
async function setupHostingerMySQL() {
  ...
  try {
    ...
    await connection.query(`
      INSERT IGNORE INTO categories (id, name, slug, description, image_url, icon) VALUES
      (1, 'Organic Fertilizers', 'organic-fertilizers', 'Bio-fertilizers & vermicompost', 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?auto=format&fit=crop&w=600&q=80', '🌿'),
      (2, 'Seeds & Gardening', 'seeds-and-gardening', 'Hybrid seeds & gardening supplies', 'https://images.unsplash.com/photo-1592417817098-8f3d6ef23a2a?auto=format&fit=crop&w=600&q=80', '🌱'),
      (3, 'Pots & Grow Bags', 'pots-and-grow-bags', 'HDPE grow bags & plastic pots', 'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?auto=format&fit=crop&w=600&q=80', '🪴');
    `);

    await connection.query(`
      INSERT IGNORE INTO collections (id, name, slug, description, image_url, category_id) VALUES
      (1, 'Best Sellers 2026', 'best-sellers', 'Top rated terrace gardening supplies', 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?auto=format&fit=crop&w=800&q=80', 1);
    `);

    await connection.query(`
      INSERT IGNORE INTO products (id, title, slug, sku, price_inr, price_usd, discount_inr, discount_usd, stock, category_id, description, status) VALUES
      (1, 'Pure Organic Vermicompost Fertilizer (5 Kg)', 'pure-organic-vermicompost-fertilizer-5kg', 'VERMI-5KG', 299.00, 5.99, 249.00, 4.99, 150, 1, 'Premium organic vermicompost rich in nitrogen, phosphorus, and microbial culture for terrace gardening.', 'Active'),
      (2, 'Cold-Pressed Bio Neem Cake Powder (1 Kg)', 'cold-pressed-bio-neem-cake-powder-1kg', 'NEEM-1KG', 199.00, 3.99, 149.00, 2.99, 85, 1, 'Natural nematicide and bio-fertilizer for pest resistance and soil conditioning.', 'Active'),
      (3, 'UV Stabilized 260 GSM HDPE Grow Bags (Pack of 5)', 'uv-stabilized-260-gsm-hdpe-grow-bags-pack-of-5', 'GB-PACK5', 499.00, 9.99, 399.00, 7.99, 200, 3, 'Heavy duty 260 GSM green grow bags designed for 5+ years of all-weather terrace gardening.', 'Active');
    `);

    await connection.query(`
      INSERT IGNORE INTO banners (id, title, subtitle, image_url, sort_order) VALUES
      (1, '100% Certified Organic Fertilizers', 'Boost your terrace garden yield naturally', 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?auto=format&fit=crop&w=1400&q=80', 1);
    `);

    await connection.query(`
      INSERT IGNORE INTO coupons (id, code, discount_type, discount_value, is_active) VALUES
      (1, 'ORGANIC15', 'PERCENT', 15, 1);
    `);

    console.log('✅ All tables & seed data successfully created/verified in Hostinger MySQL!');
